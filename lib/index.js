...
var imconvert = function(args, cb) {
  Jimp.read(args.srcData, function (err, image) {
   image.scaleToFit(args.width, args.height).quality(args.quality).getBuffer(Jimp.MIME_JPEG, cb )
  })
}
...
function convertFile(file, ext, args, files, results) {
  if (minimatch(file, args.src)) {
    ...
    if(!!args.rename_only) {
      imconvert = function(convertArgs) {
